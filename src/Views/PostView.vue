      ...
      <div class="col-xl-10">
        <div class="card shadow-sm overflow-hidden" v-if="post">
          <div class="row g-0">
            ...
            <div
              class="col-lg-7 bg-black d-flex align-items-center justify-content-center position-relative post-left-col">
              ...
              <div v-if="hasImages"
                ...
                <button v-if="post.images && post.images.length > 1" class="carousel-prev" @click="prevImage"><svg
                    xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
                    class="bi bi-chevron-left" viewBox="0 0 16 16">
                    <path fill-rule="evenodd"
                      d="M11.354 1.646a.5.5 0 0 1 0 .708L5.707 8l5.647 5.646a.5.5 0 0 1-.708.708l-6-6a.5.5 0 0 1 0-.708l6-6a.5.5 0 0 1 .708 0z" />
                  </svg></button>
                <button v-if="post.images && post.images.length > 1" class="carousel-next" @click="nextImage"><svg
                    xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
                ...
                <div v-if="post.images && post.images.length > 1" class="image-dots">
                  <span v-for="(img, i) in post.images" :key="i" :class="['dot', { active: i === imageIndex }]"
            ...
            <div class="col-lg-5 d-flex flex-column bg-white post-right-col">
              ...
              <div class="p-3 border-bottom d-flex align-items-center justify-content-between">
                <div class="d-flex align-items-center">
                  <div class="position-relative me-3">
                    <img :src="post.userAvatar || '/img01.jpg'" class="rounded-circle border" width="42" height="42"
                ...
                <div class="position-relative">
                  <button class="btn btn-link text-dark p-0" @click="showMenu = !showMenu">
                    <svg aria-label="Tùy chọn khác" class="x1lliihq x1n2onr6 x5n08af" fill="currentColor" height="24"
                  ...
                  <div v-if="showMenu" class="card position-absolute shadow-sm p-0 overflow-hidden"
                    ...
                    <div class="list-group list-group-flush text-start">
                      <button class="list-group-item list-group-item-action text-danger fw-bold" @click="reportPost">Báo
                        cáo</button>

                      <button class="list-group-item list-group-item-action" @click="goBack">Trở về bài viết</button>
                      <button class="list-group-item list-group-item-action" @click="copyLink">Sao chép liên
                        kết</button>
              ...
              <div class="flex-grow-1 overflow-auto p-3 custom-scrollbar" style="background: #fff;">
                ...
                <div class="d-flex mb-4">
                  <div class="flex-shrink-0 me-3">
                    <img :src="post.userAvatar || '/img01.jpg'" class="rounded-circle border" width="32" height="32"
                      style="object-fit: cover;" />
                  </div>
                  <div>
                    <span class="fw-bold me-1 small">{{ post.user }}</span>
                    <span class="small">{{ post.caption }}</span>
                    <div class="text-muted small mt-2" style="font-size: 12px;">{{ post.createdAt ?
                      timeAgo(post.createdAt) : post.time }}</div>
                  </div>
                </div>
                ...
                <div v-for="c in post.comments" :key="c.id" class="mb-3">
                  <div class="d-flex w-100 group-hover-visible">
                    ...
                    <div class="flex-shrink-0 me-3">
                      <div
                        class="rounded-circle bg-secondary d-flex align-items-center justify-content-center text-white border overflow-hidden"
                    ...
                    <div class="flex-grow-1">
                      ...
                      <div v-else class="position-relative">
                        ...
                        <div class="d-flex align-items-center gap-3 mt-1">
                          ...
                          <small v-if="c.likes > 0" class="text-muted fw-bold" style="font-size: 12px;">{{ c.likes }}
                            lượt thích</small>
                          <button class="btn btn-link p-0 text-muted fw-bold"
                            style="font-size: 12px; text-decoration: none;" @click="startReplyComment(c)">Trả
                            lời</button>


                          <button class="btn btn-link p-0 text-muted" style="font-size: 12px; text-decoration: none;"
                        ...
                        <div v-if="commentMenuOpen === c.id"
                          ...
                          <div class="list-group list-group-flush text-start">
                            <button v-if="c.user === me.username" class="list-group-item list-group-item-action small"
                              @click="startEditComment(c)">Chỉnh sửa</button>
                            <button v-if="c.user === me.username || post.user === me.username"
                        ...
                        <div v-if="c.replies && c.replies.length > 0" class="mt-2 ps-3 border-start">
                          <div v-for="r in c.replies" :key="r.id" class="d-flex mb-2">
                            <div
                              class="rounded-circle me-2 bg-secondary d-flex align-items-center justify-content-center text-white border overflow-hidden flex-shrink-0"
                              style="width: 24px; height: 24px; font-size: 10px;">
                              <img v-if="r.user === me.username" :src="me.avatar"
                            ...
                            <div class="position-relative">
                              ...
                              <div class="d-flex align-items-center gap-2 mt-1">
                                <small class="text-muted" style="font-size: 11px;">{{ timeAgo(r.createdAt) }}</small>
                                <button class="btn btn-link p-0 text-muted" style="font-size: 11px; text-decoration: none;" @click="toggleCommentMenu(r.id)">•••</button>
                              </div>
                              
                              <div v-if="commentMenuOpen === r.id"
                                class="card position-absolute shadow-sm p-0 overflow-hidden"
                                style="top: 100%; left: 0; z-index: 1000; min-width: 140px;">
                                <div class="list-group list-group-flush text-start">
                                  <button v-if="r.user === me.username || post.user === me.username"
                                    class="list-group-item list-group-item-action text-danger small"
                                    @click="showDeleteModal(r, c)">Xóa</button>
                                  <button v-if="r.user !== me.username"
                                    class="list-group-item list-group-item-action text-danger small"
                                    @click="reportComment(r)">Báo cáo</button>
                                  <button class="list-group-item list-group-item-action small"
                                    @click="commentMenuOpen = null">Hủy</button>
                                </div>
                              </div>
                        ...
                        <div v-if="replyingToCommentId === c.id" class="mt-2">
                          <div class="d-flex align-items-center gap-2">
                            <input v-model="replyCommentText" type="text" class="form-control form-control-sm"
                              placeholder="Viết trả lời..." @keyup.enter="submitReply(c)" />
                            <button class="btn btn-sm btn-link text-primary fw-bold text-decoration-none"
                              :disabled="!replyCommentText.trim()" @click="submitReply(c)">Đăng</button>
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div class="ms-2">
                      <button class="btn btn-link p-0 text-muted" @click="toggleCommentLike(c)">
                        <svg v-if="!isCommentLiked(c)" aria-label="Thích" class="x1lliihq x1n2onr6 x1roi4f4"
                          fill="currentColor" height="12" role="img" viewBox="0 0 24 24" width="12">
                          <path
                        ...
                        <svg v-else aria-label="Bỏ thích" class="x1lliihq x1n2onr6 x1roi4f4" fill="#ff3040" height="12"
              ...
              <div class="p-3 border-top bg-white">
                <div class="d-flex align-items-center mb-2">
                  <div class="d-flex gap-3">
                    <button class="btn btn-link p-0 text-dark" @click="toggleLike">
                      <svg v-if="!post.liked" aria-label="Thích" class="x1lliihq x1n2onr6 x5n08af" fill="currentColor"
                        height="24" role="img" viewBox="0 0 24 24" width="24">
                        <path
                          d="M16.792 3.904A4.989 4.989 0 0 1 21.5 9.122c0 3.072-2.652 4.959-5.197 7.222-2.512 2.243-3.865 3.469-4.303 3.752-.477-.309-2.143-1.823-4.303-3.752C5.141 14.072 2.5 12.167 2.5 9.122a4.989 4.989 0 0 1 4.708-5.218 4.21 4.21 0 0 1 3.675 1.941c.84 1.175.98 1.763 1.12 1.763s.278-.588 1.11-1.766a4.17 4.17 0 0 1 3.679-1.938m0-2a6.04 6.04 0 0 0-4.797 2.127 6.052 6.052 0 0 0-4.787-2.127A6.985 6.985 0 0 0 .5 9.122c0 3.61 2.55 5.827 5.015 7.97.283.246.569.494.853.747l1.027.918a44.998 44.998 0 0 0 3.518 3.018 2 2 0 0 0 2.174 0 45.263 45.263 0 0 0 3.626-3.115l.922-.824c.293-.26.59-.519.885-.774 2.334-2.025 4.98-4.32 4.98-7.94a6.985 6.985 0 0 0-6.708-7.218Z">
                        </path>
                      </svg>
                      <svg v-else aria-label="Bỏ thích" class="x1lliihq x1n2onr6 x5n08af" fill="#ff3040" height="24"
                    ...
                    <button class="btn btn-link p-0 text-dark">
                      <svg aria-label="Bình luận" class="x1lliihq x1n2onr6 x5n08af" fill="currentColor" height="24"
                        role="img" viewBox="0 0 24 24" width="24">
                    ...
                    <button class="btn btn-link p-0 text-dark">
                      <svg aria-label="Chia sẻ bài viết" class="x1lliihq x1n2onr6 x5n08af" fill="currentColor"
                        height="24" role="img" viewBox="0 0 24 24" width="24">
                        <line fill="none" stroke="currentColor" stroke-linejoin="round" stroke-width="2" x1="22"
                          x2="9.218" y1="2" y2="10.083"></line>
                  ...
                  <div class="ms-auto">
                    <button class="btn btn-link p-0 text-dark" @click="toggleBookmark">
                      <svg v-if="!post.bookmarked" aria-label="Lưu" class="x1lliihq x1n2onr6 x5n08af"
                        fill="currentColor" height="24" role="img" viewBox="0 0 24 24" width="24">
                        <polygon fill="none" points="20 21 12 13.44 4 21 4 3 20 3 20 21" stroke="currentColor"
                          stroke-linecap="round" stroke-linejoin="round" stroke-width="2"></polygon>
                      </svg>
                      <svg v-else aria-label="Đã lưu" class="x1lliihq x1n2onr6 x5n08af" fill="#fbd400" height="24"
                        role="img" viewBox="0 0 24 24" width="24">
                ...
                <div class="text-muted small text-uppercase mb-3" style="font-size: 10px;">{{ post.createdAt ?
                  timeAgo(post.createdAt) : 'Vừa xong' }}</div>


                <div class="d-flex align-items-center border-top pt-3">
                  <div class="me-2">
                    <svg aria-label="Biểu tượng cảm xúc" class="x1lliihq x1n2onr6 x5n08af" fill="currentColor"
                      height="24" role="img" viewBox="0 0 24 24" width="24">
                      <path
                  ...
                  <div class="flex-grow-1 position-relative">
                    <input v-model="newComment" class="form-control border-0 px-0 shadow-none bg-transparent"
                      placeholder="Thêm bình luận..." style="font-size: 14px;" @keyup.enter="doAddComment" />
                  </div>
...
async function loadPost() {
  ...
  try {
    ...
    if (response.data) {
      ...
      if (post.value.user === me.username) {
        post.value.userAvatar = me.avatar;
      }
      ...
      if (post.value.bookmarked === undefined) post.value.bookmarked = false;
    }
  } catch (error) {
...
function showDeleteModal(comment, parent = null) {
...
function closeDeleteModal() {
  ...
  deleteModal.parent = null;
}

async function executeDeleteComment() {
  if (!deleteModal.comment || !post.value) {
    closeDeleteModal();
    return;
  }

  if (deleteModal.parent) {
      if (deleteModal.parent.replies) {
          deleteModal.parent.replies = deleteModal.parent.replies.filter(r => r.id !== deleteModal.comment.id);
      }
  } else {
